...
/**
  * The extension allowlist here is a security control: a cover path is handed
  * back to the browser as a presigned URL, so an active document (.svg, .html)
  * would be a stored-XSS vector. These cases pin that.
  */
class CoverImageUtilsSpec extends AnyFlatSpec with Matchers {
  ...
  it should "accept unicode and spaces in the file name" in {
    CoverImageUtils.validatePathOrThrow("v1/カバー画像.jpg", maxLen) shouldBe "v1/カバー画像.jpg"
    CoverImageUtils.validatePathOrThrow("v1/my cover.png", maxLen) shouldBe "v1/my cover.png"
  }
  ...
  it should "accept a path exactly at the column length and reject one over it" in {
    // cover_image is varchar(255) on model and varchar(246) on some upgraded
    // dataset DBs; without this guard an over-long path is a 500, not a 400.
    val suffix = "/c.jpg"
    ...
    val overLimit = "v" + "a" * (256 - suffix.length - 1) + suffix
    overLimit.length shouldBe 256
    a[BadRequestException] should be thrownBy CoverImageUtils.validatePathOrThrow(overLimit, 255)
  }

  it should "honour a smaller limit for datasets" in {
    val suffix = "/c.jpg"
  ...
  "requireWithinSizeLimit" should "accept a file at exactly the limit" in {
    noException should be thrownBy
      CoverImageUtils.requireWithinSizeLimit(CoverImageUtils.SIZE_LIMIT_BYTES, "v1/cover.jpg")
  }

  it should "reject a file one byte over the limit, naming the limit in MB" in {
    val ex = intercept[BadRequestException] {
      CoverImageUtils.requireWithinSizeLimit(CoverImageUtils.SIZE_LIMIT_BYTES + 1, "v1/cover.jpg")
    }
    ex.getMessage should include("10 MB")
  }

  it should "accept a zero-byte file" in {
    noException should be thrownBy CoverImageUtils.requireWithinSizeLimit(0L, "v1/cover.jpg")
  }
}
