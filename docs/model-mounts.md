...
Python environments, and record the framework version it was trained against. Picking such
a model in the property panel surfaces a hint when the selected computing unit does not
have that environment installed, which is exactly the case where the UDF would fail at
load time or quietly load something different.
