...
import { of } from "rxjs";
import { UserModelStagedObjectsListComponent } from "./user-model-staged-objects-list.component";
...
describe("UserModelStagedObjectsListComponent", () => {
  let fixture: ComponentFixture<UserModelStagedObjectsListComponent>;
  let component: UserModelStagedObjectsListComponent;
  let getModelDiffSpy: ReturnType<typeof vi.fn>;
  let resetModelFileDiffSpy: ReturnType<typeof vi.fn>;
  ...
  beforeEach(() => {
    getModelDiffSpy = vi.fn(() => of(stagedObjects));
    resetModelFileDiffSpy = vi.fn(() => of({}));

    TestBed.configureTestingModule({
      imports: [UserModelStagedObjectsListComponent, ...commonTestImports],
      providers: [
        {
          provide: ModelService,
          useValue: { getModelDiff: getModelDiffSpy, resetModelFileDiff: resetModelFileDiffSpy },
        },
    ...
    fixture = TestBed.createComponent(UserModelStagedObjectsListComponent);
  ...
  it("fetches staged objects on init and emits them", () => {
    ...
    expect(getModelDiffSpy).toHaveBeenCalledWith(1);
    expect(component.modelStagedObjects).toEqual(stagedObjects);
  ...
  it("does not fetch staged objects when did is undefined", () => {
    ...
    expect(getModelDiffSpy).not.toHaveBeenCalled();
    expect(component.modelStagedObjects).toEqual([]);
  });
  ...
  it("coalesces bursts of change events into one refetch per audit window", () => {
    ...
    try {
      ...
      vi.advanceTimersByTime(UserModelStagedObjectsListComponent.REFRESH_AUDIT_TIME_MS);
      expect(getModelDiffSpy).toHaveBeenCalledTimes(1);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("reverts a staged object from its row's delete button", async () => {
    ...
    expect(resetModelFileDiffSpy).toHaveBeenCalledWith(1, "dir/a.txt");
    expect(getModelDiffSpy).toHaveBeenCalledTimes(2);
  });
