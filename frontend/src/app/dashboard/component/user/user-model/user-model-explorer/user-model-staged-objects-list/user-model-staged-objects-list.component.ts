...
export class UserModelStagedObjectsListComponent implements OnInit {
  ...
  @Input() set userMakeChangesEvent(event: EventEmitter<void>) {
    if (event) {
      event
        .pipe(auditTime(UserModelStagedObjectsListComponent.REFRESH_AUDIT_TIME_MS), untilDestroyed(this))
        .subscribe(() => {
          this.fetchModelStagedObjects();
        });
    }
  }
  ...
  modelStagedObjects: DatasetStagedObject[] = [];
  ...
  get stagedListHeightPx(): number {
    return Math.min(this.modelStagedObjects.length * this.STAGED_ROW_HEIGHT_PX, this.STAGED_LIST_MAX_HEIGHT_PX);
  }
  ...
  private fetchModelStagedObjects(): void {
