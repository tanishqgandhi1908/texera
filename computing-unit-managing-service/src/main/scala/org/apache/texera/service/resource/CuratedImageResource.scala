...
object CuratedImageResource extends LazyLogging {
  ...
  // Parentheses and "+" are in the set because the names administrators actually reach for
  // are of the form "Python ML (sklearn)" or "PyTorch 2.6 (CUDA 12)". Excluding them
  // rejected the obvious input with a message that read like a validation bug rather than
  // a naming rule. The set is still a deny-by-default allowlist: no quotes, no angle
  // brackets, no path or shell metacharacters, and it must start alphanumeric, so a name
  // stays safe to render and to log.
...
class CuratedImageResource extends LazyLogging {
  ...
  def create(request: CuratedImageRequest, @Auth user: SessionUser): CuratedImage = {
    ...
    // Registering a reference that is already curated is almost always a mistake -- the
    // same link pasted twice under two names. It is not free: the registry deduplicates
    // the blobs, but the mirror job still pulls the whole image from upstream again,
    // which was measured at ~4.3 GB and ~5 minutes for a real image. Naming the existing
    // row lets the administrator use it or refresh it instead.
