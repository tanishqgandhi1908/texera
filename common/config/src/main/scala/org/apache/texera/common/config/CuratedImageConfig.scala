...
object CuratedImageConfig {
  ...
  /**
    * Registry the mirrored copies live in, and the address a computing-unit pod pulls
    * from.
    *
    * This is an address rather than a Kubernetes service name on purpose: the component
    * that resolves it is the node's kubelet, pulling the image before the pod exists, and
    * kubelet resolves through the node's DNS rather than the cluster's. A
    * `*.svc.cluster.local` name is therefore not resolvable at the moment it is needed,
    * so the registry Service is given a fixed address and that address is named here.
    */
