...
trait PythonUdfUiParameterSupport {
  ...
  protected final def injectUiParameters(code: String): String =
    PythonUdfUiParameterSupport.injectForCompilation(code, uiParameters)

  /**
    * The setup this operator defers to execution time, or None when it names no resources
    * and so has nothing to defer.
    */
  protected final def uiParameterExecutionBinding(code: String): Option[ExecutionTimeBinding] =
    PythonUdfUiParameterSupport.executionBinding(code, uiParameters)
}

/**
  * Handling of a Python UDF's UI parameters that is shared with
